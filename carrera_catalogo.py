...
class Vcarrera(QWidget):
    def __init__(self):
        super(Vcarrera, self).__init__()
    ...
    def generartabla(self):
        ...
        self.agregar = QPushButton('Agregar Carrera')
    ...
    def pedir_nombre(self):
        ...
        result = ventanita.exec()
        if result == QDialog.DialogCode.Accepted:
            carre = ventanita.carrera.text()
            id = ventanita.Id.text()
            self.agregar_fila(carre, id)
...
class emergente(QDialog):
    def __init__(self):
        ...
        self.carreraP=QLabel('Ingrese la carrera:',self)
        self.carreraP.setGeometry(10,25,160,15)
        self.carrera=QLineEdit(self)
    ...
    def campos(self):
        carrera_texto = self.carrera.text().strip()
        id_texto = self.Id.text().strip()

        if not carrera_texto or not id_texto:
            stilo = """QDialog {background-color: #072d44}"""
            self.setStyleSheet(stilo)
...
class emergente2(QDialog):
    def __init__(self):
        ...
        self.close_button.setGeometry(220, 0, 30, 30)
        self.setFixedSize(250, 230) 
        self.carreraP=QLabel('Ingrese la nueva carrera:',self)
        self.carreraP.setGeometry(10,15,200,15)
        self.carrera=QLineEdit(self)
        self.Idl = QLabel('Ingrese el nuevo Id:', self)
        self.Idl.setGeometry(10,62,160,15)
        self.Id = QLineEdit(self)  
        self.Idl2=QLabel('Ingrese el id de la carrera a cambiar:', self)
