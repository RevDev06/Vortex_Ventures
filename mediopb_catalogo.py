...
class Vmediopb(QWidget):
    def __init__(self):
        super(Vmediopb, self).__init__()
    ...
    def generartabla(self):
        ...
        self.agregar = QPushButton('Agregar Medio de Publicidad')
    ...
    def nuevosdatos(self):
        ...
        if result == QDialog.DialogCode.Accepted:
            nombre = ventanita.mediopub.text()
    ...
    def pedir_nombre(self):
        ...
        result = ventanita.exec()
        if result == QDialog.DialogCode.Accepted:
            mediopub = ventanita.mediopub.text()
            id = ventanita.Id.text()
            self.agregar_fila(mediopub, id)
...
class emergente(QDialog):
    def __init__(self):
        ...
        self.mediopubP=QLabel('Ingrese el medio de publicidad:',self)
        self.mediopubP.setGeometry(10,25,160,15)
        self.mediopub=QLineEdit(self)
        ...
        layout = QVBoxLayout()
        layout.addWidget(self.mediopub)
        layout.addWidget(self.Id)
        layout.addWidget(verifica)
        self.setLayout(layout)

    def campos(self):
        mediopub_texto = self.mediopub.text().strip()
        id_texto = self.Id.text().strip()

        if not mediopub_texto or not id_texto:
            stilo = """QDialog {background-color: #072d44}"""
            self.setStyleSheet(stilo)
...
class emergente2(QDialog):
    def __init__(self):
        ...
        self.close_button.setGeometry(220, 0, 30, 30)
        self.setFixedSize(250, 230) 
        self.mediopubP=QLabel('Ingrese el nuevo medio de publicidad:',self)
        self.mediopubP.setGeometry(10,15,250,15)
        self.mediopub=QLineEdit(self)
        self.Idl = QLabel('Ingrese el nuevo Id:', self)
        self.Idl.setGeometry(10,62,160,15)
        self.Id = QLineEdit(self)  
        self.Idl2=QLabel('Ingrese el id del medio de publicidad a cambiar:', self)
