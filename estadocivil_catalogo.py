...
class Vestadocivil(QWidget):
    def __init__(self):
        super(Vestadocivil, self).__init__()
    ...
    def generartabla(self):
        ...
        self.agregar = QPushButton('Agregar Estado Civil')
    ...
    def pedir_nombre(self):
        ...
        result = ventanita.exec()
        if result == QDialog.DialogCode.Accepted:
            estado = ventanita.estadoc.text()
            id = ventanita.Id.text()
            self.agregar_fila(estado, id)
...
class emergente(QDialog):
    def __init__(self):
        ...
        self.estadocP=QLabel('Ingrese el estado civil:',self)
        self.estadocP.setGeometry(10,25,160,15)
        self.estadoc=QLineEdit(self)
        ...
        layout = QVBoxLayout()
        layout.addWidget(self.estadoc)
        layout.addWidget(self.Id)
        layout.addWidget(verifica)
        self.setLayout(layout)

    def campos(self):
        estado_texto = self.estadoc.text().strip()
        id_texto = self.Id.text().strip()

        if not estado_texto or not id_texto:
            stilo = """QDialog {background-color: #072d44}"""
            self.setStyleSheet(stilo)
...
class emergente2(QDialog):
    def __init__(self):
        ...
        self.close_button.setGeometry(220, 0, 30, 30)
        self.setFixedSize(250, 230) 
        self.estadocP=QLabel('Ingrese el nuevo estado civil:',self)
        self.estadocP.setGeometry(10,15,200,15)
        self.estadoc=QLineEdit(self)
        self.Idl = QLabel('Ingrese el nuevo Id:', self)
        self.Idl.setGeometry(10,62,160,15)
        self.Id = QLineEdit(self)  
        self.Idl2=QLabel('Ingrese el id del estado civil a cambiar:', self)
