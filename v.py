...
from PyQt6.QtWidgets import (QApplication, QWidget, QLabel, QLineEdit, QPushButton, QDateTimeEdit,
                             QMessageBox, QCheckBox, QComboBox, QVBoxLayout, QRadioButton)
...
# fecha de elaboracion
        ...
        # se muestra como verdadero  la creacion del calendario
        fecha_elaboracion.setCalendarPopup(True)
        # para que en la barra se acomoden correctamente las fechas o en como va estar el formato
        fecha_elaboracion.setDisplayFormat("yyyy/MM/dd")
        # muestra primero el año que es "yyyy", y haci va señalando la posicion de la fechas
        # setLocale para que el calendario este en español poniendo "es_ES"
        fecha_elaboracion.calendarWidget().setLocale(QLocale("es_ES"))
        # se utiliza para poder actualizar el calendario y muestre la fecha actual
        fecha_elaboracion.setDateTime(QDateTime.currentDateTime())
        # se hace para poder realizar la actualizacion de dia,mes y hora
        fecha_elaboracion.calendarWidget().clicked.connect(self.actualizar_fecha_hora)
        # aqui para actualizar el año del calendario
        fecha_elaboracion.dateChanged.connect(self.actualizar_anio)
...
# vacante generada por:
        ...
        # son las coordenadas de donde se va a localizar
        vacante_button3.move(200, 294)
...
# botton de enviar
    ...
    def on_toggle(self):
        ...
        if sender.isChecked():  # y que cuando se seleccione se muestre por consola
            # diciendo el nombre del button que se a seleccionado
            print(f'Se selecciono: {sender.text()}')
# calendario

    def actualizar_fecha_hora(self, date):  # subclase para la actualizacion
        fecha_hora_actual = self.sender().selectedDate().toDateTime(
            fecha_elaboracion.time())  # funcion para tener el tiempo actual
        # se llama la variable de actualizacion para completar la funcion de tiempo actual de calendario
        fecha_elaboracion.setDateTime(fecha_hora_actual)

    def actualizar_anio(self, date):
        anio_actual = date.year()  # aqui para actualizar el año
        fecha_elaboracion.setDate(date)
        # para complete la funcion de poner el año correctamente
        fecha_elaboracion.setTime(fecha_elaboracion.time())
