...
class Vgradoavnce(QWidget):
    def __init__(self):
        super(Vgradoavnce, self).__init__()
    ...
    def generartabla(self):
        ...
        self.agregar = QPushButton('Agregar Grado Avance')
    ...
    def buscar(self,idbuscar,nnombre,nid):
        for fila in range(self.tableWidget.rowCount()):
            ...
            if idenfila == idbuscar:
                ...
                QMessageBox.warning(self, "", "No esxiste ningun grado avance con esa Id")
    ...
    def pedir_nombre(self):
        ...
        result = ventanita.exec()
        if result == QDialog.DialogCode.Accepted:
            gradoa = ventanita.gradoava.text()
            id = ventanita.Id.text()
            self.agregar_fila(gradoa, id)
...
class emergente(QDialog):
    def __init__(self):
        ...
        self.gradoavaP=QLabel('Ingrese el Grado Avance:',self)
        self.gradoavaP.setGeometry(10,25,160,15)
        self.gradoava=QLineEdit(self)
        ...
        layout = QVBoxLayout()
        layout.addWidget(self.gradoava)
        layout.addWidget(self.Id)
        layout.addWidget(verifica)
        self.setLayout(layout)

    def campos(self):
        grado_texto = self.gradoava.text().strip()
        id_texto = self.Id.text().strip()

        if not grado_texto or not id_texto:
            stilo = """QDialog {background-color: #072d44}"""
            self.setStyleSheet(stilo)
...
class emergente2(QDialog):
    def __init__(self):
        ...
        self.close_button.setGeometry(220, 0, 30, 30)
        self.setFixedSize(250, 230) 
        self.gradoavaP=QLabel('Ingrese el nuevo grado avance:',self)
        self.gradoavaP.setGeometry(10,15,200,15)
        self.gradoava=QLineEdit(self)
        self.Idl = QLabel('Ingrese el nuevo Id:', self)
        self.Idl.setGeometry(10,62,160,15)
        self.Id = QLineEdit(self)  
        self.Idl2=QLabel('Ingrese el id del grado avance a cambiar:', self)
